...
.role-indicator{background:linear-gradient(135deg,var(--primary),var(--primary-dark));color:#fff;padding:12px 20px;border-radius:12px;text-align:center;margin-bottom:20px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:8px}
</style>
</head>
  ...
  <div class="auth-card">
    <c:if test="${not empty selectedRole}">
      <div class="role-indicator">
        <c:if test="${selectedRole == 'SELLER'}">
          <i class="fas fa-store"></i> Signing in as Seller
        </c:if>
        <c:if test="${selectedRole == 'BUYER'}">
          <i class="fas fa-shopping-cart"></i> Signing in as Buyer
        </c:if>
      </div>
    </c:if>
    <h2>Welcome Back</h2>
