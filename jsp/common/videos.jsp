...
body.videos-body{background:#000;overflow:hidden;touch-action:pan-y}
.vpage{display:flex;height:100vh;width:100vw}

/* Feed */
.vfeed-wrap{flex:1;height:100vh;overflow-y:scroll;scroll-snap-type:y mandatory;position:relative}
.vfeed-wrap::-webkit-scrollbar{display:none}
.vslide{height:100vh;scroll-snap-align:start;position:relative;display:flex;align-items:center;justify-content:center;background:#111;touch-action:pan-y}
.vplayer{width:100%;height:100%;max-width:420px;object-fit:contain;cursor:pointer;display:block;transition:transform 0.3s ease}
.vslide.swiping-up .vplayer{transform:scale(0.95) translateY(20px);opacity:0.8}
.vslide.swiping-down .vplayer{transform:scale(0.95) translateY(-20px);opacity:0.8}
...
function handleTouchMove(e) {
  ...
  var currentSlide = document.querySelector('.vslide[data-idx="' + currentIdx + '"]');
  
  if (currentSlide) {
    currentSlide.classList.remove('swiping-up', 'swiping-down');
    if (deltaY < -50) {
      currentSlide.classList.add('swiping-up');
    } else if (deltaY > 50) {
      currentSlide.classList.add('swiping-down');
    }
  }
}
...
function handleTouchEnd(e) {
  ...
  // Swipe up - next video
  if (deltaY < -80 && currentIdx < document.querySelectorAll('.vslide').length - 1) {
    scrollToVideo(currentIdx + 1);
  }
  // Swipe down - previous video  
  else if (deltaY > 80 && currentIdx > 0) {
    scrollToVideo(currentIdx - 1);
  }
}

function scrollToVideo(index) {
  var feedWrap = document.getElementById('vfeedWrap');
  var targetSlide = document.getElementById('vslide-' + index);
  if (feedWrap && targetSlide) {
...
document.addEventListener('DOMContentLoaded', function() {
  var feedWrap = document.getElementById('vfeedWrap');
  if (feedWrap) {
    feedWrap.addEventListener('touchstart', handleTouchStart, { passive: true });
    feedWrap.addEventListener('touchmove', handleTouchMove, { passive: true });
    feedWrap.addEventListener('touchend', handleTouchEnd, { passive: true });
  }
});
